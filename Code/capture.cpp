...
char filename[] = "./sdcard/imoge_0000.ppm";
char filesave[] = "./onchip/image_0000.jpg";
char jitter[] = "./onchip/plot.csv";

char username[20];
char host_name[20];

struct timespec start,end,abs_start,abs_end,start2,end2,nano;
...
{ 
	...
	VideoWriter writer("./sdcard/video.avi",CV_FOURCC('D','I','V','X'),20,Size(640,480));
...
int main ()
{
	...
	/*Create Threads*/
	rc = pthread_create( &captureImage_thread , &captureImage_attr, captureImage , ( void * ) 0 ) ;
	if (rc)
   	{
	...
	rc = pthread_create( &saveImage_thread , &saveImage_attr, saveImage, ( void * ) 0 ) ;
	if (rc)
   	{
